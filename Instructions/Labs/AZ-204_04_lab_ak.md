...
1.  Microsoft 계정의 암호를 입력한 다음 로그인을 선택합니다.
    ...
    > **참고**: Azure Portal에 처음 로그인하는 경우 포털 둘러보기 기능이 제공됩니다. 둘러보기를 건너뛰고 포털 사용을 시작하려면 시작하기를 선택합니다.
...
1.  **SQL Database 서버 만들기** 블레이드에서 **기본**, **네트워킹**, **추가 설정** 등의 블레이드 탭을 살펴봅니다.

    > **참고**: 각 탭은 새 Azure SQL Database 서버를 만드는 워크플로의 단계를 나타냅니다. 언제든지 검토 + 만들기를 선택하여 나머지 탭을 건너뛸 수 있습니다.

    1.  기본 탭에서 다음 작업을 수행합니다.
    
    1.  **구독** 드롭다운 목록의 값은 기본값으로 설정된 상태로 유지합니다.
    
    1.  **리소스 그룹** 섹션에서 **새로 만들기**를 선택하고 **PolyglotData**를 입력한 다음 **확인**을 선택합니다.
    
    1.  **서버 이름** 텍스트 상자에 **polysqlsrvr*[사용자 이름]*** 을 입력합니다.
...
1.  **Azure Cosmos DB 계정 만들기** 블레이드에서 **기본**, **네트워크**, **태그** 등의 블레이드 탭을 살펴봅니다.

    > **참고**: 각 탭은 새 Azure Cosmos DB 계정을 만드는 워크플로의 단계를 나타냅니다. 언제든지 **검토 + 만들기**를 선택하여 나머지 탭을 건너뛸 수 있습니다.

1.  **기본** 탭에서 다음 작업을 수행합니다.
    
    1.  **구독** 목록의 값은 기본값으로 설정된 상태로 유지합니다.
    
    1.  **리소스 그룹** 섹션의 목록에서 **PolyglotData**를 선택합니다.
    
    1.  **계정 이름** 텍스트 상자에 **polycosmos*[사용자 이름]*** 을 입력합니다.
...
1.  **PolyglotData** 블레이드에서 이 랩의 앞부분에서 만든 **polycosmos*[사용자 이름]*** Azure Cosmos DB 계정을 선택합니다.
...
1.  모든 서비스 블레이드에서 스토리지 계정을 선택합니다.

1.  **스토리지 계정** 블레이드에서 스토리지 인스턴스 목록을 확인합니다.

1.  스토리지 계정 블레이드에서 새로 만들기를 선택합니다.

1.  **스토리지 계정 만들기** 블레이드에서 **기본**, **고급**, **태그** 등의 블레이드 탭을 살펴봅니다.
...
1.  **기본** 탭에서 다음 작업을 수행합니다.
    ...
    1.  **스토리지 계정 이름** 텍스트 상자에 **polystor*[사용자 이름]*** 을 입력합니다.
...
1.  **PolyglotData** 블레이드에서 이 랩의 앞부분에서 만든 **polystor*[사용자 이름]*** 스토리지 계정을 선택합니다.

1.  **스토리지 계정** 블레이드에서 **Blob service** 섹션에 있는 **컨테이너** 링크를 선택합니다.

1.  **컨테이너** 섹션에서 **+ 컨테이너**를 선택합니다.

1.  **새 컨테이너**에서 다음 작업을 수행합니다.
...
1.  **Blob 업로드** 팝업 창에서 다음 작업을 수행합니다.
    ...
    1.  **파일 탐색기** 창에서 **Allfiles (F):\\Allfiles\\Labs\\04\\Starter**로 이동하여 **AdventureWorks.bacpac** 파일을 선택한 다음 **열기**를 선택합니다.
...
1.  **PolyglotData** 블레이드에서 이 랩의 앞부분에서 만든 **polysqlsrvr*[사용자 이름]*** SQL Server를 선택합니다.
...
1.  **데이터베이스 가져오기** 블레이드에서 다음 작업을 수행합니다.
    ...
    1.  **스토리지 계정** 블레이드에서 이 랩의 앞부분에서 만든 **polystor*[사용자 이름]*** 스토리지 계정을 선택합니다. 
    ...
    1.  서버 관리자 로그인 텍스트 상자에 testuser를 입력합니다.
    
    1.  암호 텍스트 상자에 TestPa55w.rd를 입력합니다.
...
1.  **PolyglotData** 블레이드에서 이 랩의 앞부분에서 만든 **AdventureWorks** SQL 데이터베이스를 선택합니다.
...
1.  실행을 선택하여 쿼리를 실행한 다음 결과를 살펴봅니다.
...
1.  3줄의 JSON(JavaScript Object Notation) 개체에서 **ConnectionStrings.AdventureWorksSqlContext** 경로를 찾습니다. 현재 값이 비어 있는지 확인합니다.
...
1.  이 랩의 앞부분에서 기록한 SQL 데이터베이스의 **ADO.NET(SQL 인증) 연결 문자열**로 값을 설정하여 **AdventureWorksSqlContext** 속성의 값을 업데이트합니다.
...
1.  이 랩의 앞부분에서 기록한 Azure Storage Blob 컨테이너 **images**의 **URL** 속성으로 값을 설정하여 **BlobContainerUrl** 속성의 값을 업데이트합니다.
...
1.  Water Bottle 모델을 찾아 세부 정보 보기를 선택합니다.

1.  Water Bottle 제품 세부 정보 페이지에서 장바구니에 추가를 찾은 다음 체크 아웃 기능이 현재 비활성화되어 있는지 확인합니다.
...
1.  명령 프롬프트에서 다음 명령을 입력하고 Enter 키를 눌러 NuGet에서 **Microsoft.EntityFrameworkCore.SqlServer**의 버전 2.2.6을 가져옵니다.

    ```
    dotnet add package Microsoft.EntityFrameworkCore.SqlServer --version 3.0.1
    ```

    > **참고**: **dotnet add package** 명령은 NuGet에서 **Microsoft.EntityFrameworkCore.SqlServer** 패키지를 추가합니다. 자세한 내용은 다음을 참조하세요. [Microsoft.EntityFrameworkCore.SqlServer](https://www.nuget.org/packages/Microsoft.EntityFrameworkCore.SqlServer/3.0.1).

1.  명령 프롬프트에서 다음 명령을 입력하고 Enter 키를 눌러 NuGet에서 **Microsoft.Azure.Cosmos**의 버전 3.4.1을 가져옵니다.

    ```
    dotnet add package Microsoft.Azure.Cosmos --version 3.4.1
    ```

    > **참고**: dotnet add package 명령은 NuGet에서 Microsoft.Azure.Cosmos 패키지를 추가합니다. 자세한 내용은 다음을 참조하세요. [Microsoft.Azure.Cosmos](https://www.nuget.org/packages/Microsoft.Azure.Cosmos/3.4.1).
...
1.  **Visual Studio Code** 창의 탐색기 창에서 **AdventureWorks.Migrate** 프로젝트를 확장합니다.

1.  **Program.cs** 파일을 엽니다.

1.  **Program.cs** 파일의 코드 편집기 탭에서 기존 파일의 모든 코드를 삭제합니다.

1.  다음 코드 줄을 추가하여 참조 대상 **AdventureWorks.Models** 및 **AdventureWorks.Context** 프로젝트에서 **AdventureWorks.Models** 및 **AdventureWorks.Context** 네임스페이스를 가져옵니다.
...
1.  **Program** 클래스 내에서 다음 코드 줄을 입력하여 새 문자열 상수 **sqlDBConnectionString**을 만듭니다.

    ```
    private const string sqlDBConnectionString = "";
    ```

1.  이 랩의 앞부분에서 기록한 SQL 데이터베이스의 ADO.NET(SQL 인증) 연결 문자열에 해당 값을 설정하여 sqlDBConnectionString 문자열 상수를 업데이트합니다.

    > **참고**: 여기에서는 업데이트된 연결 문자열을 사용하는 것이 중요합니다. 포털에서 복사한 원본 연결 문자열에는 SQL 데이터베이스에 연결하는 데 필요한 사용자 이름과 암호가 없습니다.

1.  **Program** 클래스 내에서 다음 코드 줄을 입력하여 새 문자열 상수 **cosmosDBConnectionString**을 만듭니다. 

    ```
    private const string cosmosDBConnectionString = "";
    ```

1.  이 랩의 앞부분에서 기록한 Azure Cosmos DB 계정의 기본 연결 문자열로 값을 설정하여 cosmosDBConnectionString 문자열 상수를 업데이트합니다.
...
1.  **Main** 메서드 내에서 콘솔에 소개 메시지를 출력하는 다음 코드 줄을 추가합니다.
...
1.  **Program.cs** 파일 내 **Program** 클래스의 **Main** 메서드 내에서 다음 코드 줄을 추가하여 **AdventureWorksSqlContext** 클래스의 새 인스턴스를 만듭니다. 연결 문자열 값으로는 *sqlDBConnectionString* 변수를 전달합니다.

    ```
    using AdventureWorksSqlContext context = new AdventureWorksSqlContext(sqlDBConnectionString);
    ```

1.  **Main** 메서드 내에서 다음 코드 블록을 추가합니다. 이 코드 블록은 LINQ(Language-Integrated Query)를 실행하여 데이터베이스의 모든 **Models** 및 자식 **Products**를 가져와 메모리 내 **List<>** 컬렉션에 저장합니다.
...
1.  **Main** 메서드 내에서 SQL Database에서 가져온 레코드 수를 인쇄하는 다음 코드 줄을 추가합니다.
...
1.  **Program.cs** 파일 내 **Program** 클래스의 **Main** 메서드 내에서 다음 코드 줄을 추가하여 **CosmosClient** 클래스의 새 인스턴스를 만듭니다. 연결 문자열 값으로는 *cosmosDBConnectionString* 변수를 전달합니다.

    ```
    using CosmosClient client = new CosmosClient(cosmosDBConnectionString);
    ```

1.  **Main** 메서드 내에서 Azure Cosmos DB 계정에 **데이터베이스** **Retail**이 없으면 새로 만드는 다음 코드 줄을 추가합니다.

    ```
    Database database = await client.CreateDatabaseIfNotExistsAsync("Retail");
    ```

1.  **Main** 메서드 내에서 다음 코드 블록을 추가합니다. 이 코드 블록은 Azure Cosmos DB 계정에 **컨테이너** **Online**이 없으면 파티션 키 경로를 **/Category**로, 요청 단위 처리량을 **1000**으로 설정하여 새로 만듭니다.
...
1.  **Main** 메서드 내에서 *int* 변수  **count**를 만드는 다음 코드 줄을 추가합니다.

    ```
    int count = 0;
    ```

1.  **Main** 메서드 내에서 다음 코드 블록을 추가합니다. 이 코드 블록은 **items** 컬렉션의 개체에 대해 반복 실행되는 **foreach** 루프를 만듭니다.
...
1.  **Main** 메서드의 **foreach** 루프 내에서 다음 코드 줄을 추가합니다. 이 코드 줄은 Azure Cosmos DB 컬렉션에 개체를 **upsert**하고 *ItemResponse<>* 형식 변수 **document**에 결과를 저장합니다.

    ```
    ItemResponse<Model> document = await container.UpsertItemAsync<Model>(item);
    ```

1.  **Main** 메서드에 포함된 **foreach** 루프 내에서 각 upsert 작업의 활동 ID를 인쇄하는 다음 코드 줄을 추가합니다.

    ```
    await Console.Out.WriteLineAsync($"Upserted document #{++count:000} [Activity Id: {document.ActivityId}]");
    ```

1.  다시 **Main** 메서드 내에서(**foreach** 루프 외부) Azure Cosmos DB로 내보낸 문서 수를 인쇄하는 다음 코드 줄을 추가합니다.
...
1.  바로 가기 메뉴에 액세스하거나, **AdventureWorks.Context** 폴더 노드를 마우스 오른쪽 단추로 클릭하거나 해당 폴더 노드의 바로 가기 메뉴를 활성화한 다음 **새 파일**을 선택합니다.
...
1.  **AdventureWorksCosmosContext** 클래스 내에서 다음 코드 줄을 입력하여 새 읽기 전용 *Container* 변수 **_container**를 만듭니다.
...
1.  **FindModelAsync** 메서드 내에서 다음 코드 블록을 추가합니다. 이 코드 블록은 LINQ 쿼리를 만들어 반복기로 변환한 다음, 결과 집합에 대해 쿼리를 반복 실행하여 결과 집합의 항목 하나를 반환합니다.
...
1.  **GetModelsAsync** 메서드 내에서 다음 코드 블록을 추가합니다. 이 코드 블록은 SQL 쿼리를 실행하고 쿼리 결과 반복기를 가져온 다음, 결과 집합에 대해 쿼리를 반복 실행하여 모든 결과의 합집합을 반환합니다.
...
1.  **FindProductAsync** 메서드 내에서 다음 코드 블록을 추가합니다. 이 코드 블록은 SQL 쿼리를 실행하고 쿼리 결과 반복기를 가져온 다음, 결과 집합에 대해 쿼리를 반복 실행하여 결과 집합의 항목 하나를 반환합니다.
...
1.  **ConfigureProductService** 메서드 내에서 다음 코드 블록을 추가하여 이 랩의 앞부분에서 만든 **AdventureWorksCosmosContext** 구현으로 제품 공급자를 변경합니다.
...
1.  Touring-1000 모델을 찾아 세부 정보 보기를 선택합니다.
...
1.  명령 프롬프트에서 다음 명령을 입력하고 Enter 키를 눌러 **PolyglotData** 리소스 그룹을 삭제합니다.
