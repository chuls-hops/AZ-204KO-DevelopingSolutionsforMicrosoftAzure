...
이 연습에서는 함수 앱에 대한 시스템 할당 관리 서비스 ID를 만든 다음 해당 ID에 키 자격 증명 모음에서 비밀 값을 얻을 수 있는 적절한 권한을 부여했습니다. 마지막으로 함수 앱 내에서 사용할 비밀을 만들었습니다.
...
1.  업데이트된 함수 코드를 **저장**합니다.

1.  **테스트/실행**을 선택하여 구성할 수 있는 팝업 대화 상자를 열고 함수 테스트를 수행합니다. 실행의 출력은 "테스트 성공"이어야 합니다.
...
1.  **테스트/실행**을 선택하여 구성할 수 있는 팝업 대화 상자를 열고 함수 테스트를 수행합니다. 실행의 출력은 Key Vault에 저장된 스토리지 계정 연결 문자열이어야 합니다.
...
1.  이 랩의 앞에서 만든 **securestor*[yourname]*** 스토리지 계정에 액세스합니다.
...
1.  새 **드롭** 컨테이너로 이동한 다음 **업로드**를 선택하여 **Allfiles (F):** 에서 **records.json** 파일을 업로드합니다.랩 가상 머신의 **\\Allfiles\\Labs\\07\\Starter** 폴더
...
1.  다음 세부 정보를 사용하여 새 애플리케이션 설정을 만듭니다.
    ...
    - 이름: **DOTNET_ADD_GLOBAL_TOOLS_TO_PATH**
...
1.  새 브라우저 창에서 **App Service 편집기**를 엽니다.

1.  **App Service 편집기** 브라우저 창에서 **FileParser** 폴더 내에 **function.proj**라는 새 파일을 만듭니다.

1.  편집기를 사용하여 이 구성 내용을 삽입합니다.
...
1.  **App Service** 블레이드에서 **FileParser** 함수에 대한 편집기를 찾아 엽니다.

1.  **Run** 메서드 내의 모든 기존 코드를 삭제합니다.

1.  **Azure.Storage**, **Azure.Storage.Blobs** 및 **Azure.Storage.Blobs.Models** 네임스페이스에 대한 세 개의 **using** 지시문을 추가합니다.
...
1.  **records.json** Blob 이름을 전달하는 동안 **BlobContainerClient.GetBlobClient** 메서드를 사용하여 이 랩의 이전에 업로드한 Blob을 참조하는 **BlobClient** 클래스의 새 인스턴스를 만듭니다.
...
1.  **테스트/실행**을 선택하여 구성할 수 있는 팝업 대화 상자를 열고 함수 테스트를 수행합니다. 실행의 출력은 스토리지 계정에 저장된 **$/drop/records.json Blob** 의 콘텐츠이어야 합니다.
