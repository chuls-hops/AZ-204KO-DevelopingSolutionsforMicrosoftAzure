...
1.  SSH 도구는 먼저 호스트의 진위를 확인할 수 없다는 것을 알려주고 계속 연결을 원하는지 묻습니다. **예**를 입력한 다음 엔터를 선택하여 VM에 계속 연결합니다.
...
1.  **기본** 탭에서 다음 작업을 수행합니다.
    ...
    1.  **이미지 레지스트리 로그인 서버** 텍스트 상자에 이전에 랩에서 기록한 **로그인 서버** 값을 입력합니다.
...
1.  **네트워킹** 탭에서 다음 작업을 수행합니다.
    ...
    1.  **포트** 섹션에서 포트가 **80**이고 포트 프로토콜이 **TCP**인지 확인합니다.
...
1.  **고급** 탭에서 다음 작업을 수행합니다.
    ...
    1.  **정책 재시작** 드롭다운 목록에서 **실패 시**를 선택합니다.
    
    1.  **환경 변수**텍스트 상자를 비워 둡니다.
