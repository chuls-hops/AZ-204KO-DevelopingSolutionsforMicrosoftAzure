...
1.  **기본** 탭에서 다음 작업을 수행합니다.
    ...
    1.  **영역** 드롭다운 목록에서 **(US) 미국 동부** 위치를 선택합니다.
...
1.  포털에서 **Cloud Shell** 아이콘을 선택하여 새 셸 인스턴스를 엽니다.
    ...
    > **참고**: **Cloud Shell**아이콘은 상단에 ">_" 로 표시됩니다.
...
1.  다음 명령을 입력하고 엔터를 선택하여 **가상 머신 만들기** 명령에 대한 예와 인수 목록을 가져옵니다.

    ```
    az vm create --help
    ```

1.  다음 명령을 입력하고 엔터를 선택하여 다음 설정으로 새 **가상 머신**을 만듭니다.
...
1.  다음 명령을 입력하고 엔터를 선택하여 출력을 필터링하고 첫 번째 IP 주소 값만 반환합니다.
...
1.  다음 명령을 입력하고 엔터를 선택하여 Bash 셸 변수 *ipAddress*의 값을 렌더링합니다.

    ```
    echo $ipAddress
    ```

1.  다음 명령을 입력하고 엔터를 선택하여 SSH(Secure Shell) 도구 및 Bash 셸 변수 *ipAddress*에 저장된 IP 주소를 사용하여 이전에 랩에서 만든 VM에 연결합니다.

    ```
    ssh student@$ipAddress
    ```

1.  SSH 도구는 먼저 호스트의 진위를 확인할 수 없다는 것을 알려주고 계속 연결을 원하는지 묻습니다. **yes**를 입력한 다음 엔터를 선택하여 VM에 계속 연결합니다.

1.  SSH 도구가 암호를 묻습니다. **StudentPa55w.rd**를 입력한 다음 엔터를 선택하여 VM으로 인증합니다.
...
1.  포털의 **Cloud Shell** 명령 프롬프트에서 다음 명령을 입력하고 엔터를 선택하여 루트 디렉터리에서 **\~/clouddrive**디렉터리로 이동합니다.

    ```
    cd ~/clouddrive
    ```

1.  다음 명령을 입력하고 엔터를 선택하여 **\~/clouddrive** 디렉터리에서 **ipcheck**라는 새 디렉터리를 만듭니다.

    ```
    mkdir ipcheck
    ```

1.  다음 명령을 입력하고 엔터를 선택하여 active directory를 **\~/clouddrive**에서 **\~/clouddrive/ipcheck**로 변경합니다.
...
1.  다음 명령을 입력하고 엔터를 선택하여 현재 디렉터리 컨텍스트에 포함된 그래픽 편집기를 엽니다.
...
1.  검색 텍스트 상자에 **Container Registry**를 입력하고 Enter 키를 누릅니다.
...
1.  포털의 **Cloud Shell** 명령 프롬프트에서 다음 명령을 입력하고 엔터를 선택하여 구독의 모든 컨테이너 레지스트리 목록을 얻습니다.
...
1.  다음 명령을 입력하고 엔터를 선택하여 컨테이너 레지스트리에 소스 코드를 업로드하고 컨테이너 이미지를 Container Registry 작업으로 빌드합니다.
...
1.  **ContainerCompute** 블레이드에서 이 랩 앞 부분에서 만든 컨테이너 레지스트리를 선택합니다.
...
### 연습 3: Azure Container Instances를 배포합니다. 
...
1.  **Container Registry** 블레이드 개요 상단 메뉴에서 **업데이트**를 선택합니다.

1.  **컨테이너 레지스트리 업데이트** 블레이드에서 다음 작업을 수행합니다.
    
    -   **관리 사용자** 섹션에서 **사용 설정**을 선택하고 **저장**을 선택합니다.
...
1.  **컨테이너 레지스트리**블레이드에서 **설정** 섹션을 찾은 다음 **액세스 키** 링크를 선택합니다.
...
1.  **액세스 키** 섹션에서 다음 필드의 값을 기록합니다.
    ...
    > **참고**: 나중에 이 랩에서 다른 컨테이너 인스턴스를 만들 때 이러한 값을 사용합니다.
...
1.  **기본** 및 **고급**과 같은 **컨테이너 인스턴스 만들기** 블레이드에서 탭을 찾습니다.
...
1.  **기본** 탭에서 다음 작업을 수행합니다.
    ...
    1.  **지역** 드롭다운 목록에서 **(US) 미국 동부**를 선택합니다.
    
    1.  **이미지 소스**는 **Azure 컨테이너 레지스트리**을 선택합니다.

    1.  **레지스트리** 섹션에서 드롭다운에서 **앞에서 만든 컨테이너 레지스트리**를 선택합니다.

    1.  **이미지** 섹션에서 드롭다운에서 **ipcheck**를 선택합니다.
    
    1.  **이미지 태그** 섹션에서 드롭다운에서 **latest**를 선택합니다.
