...
1.  **raster-graphics** 컨테이너에서 **업로드** 를 선택하여 랩 VM의 다음 폴더에 있는 **graph.jpg** 파일을 업로드합니다. **Allfiles (F): \\Allfiles\\Labs\\03\\Starter\\Images**.
...
1.  **vector-graphics** 컨테이너에서 **업로드** 를 선택하여 **Allfiles (F):** 에 **graph.svg** 파일을 업로드합니다.**Allfiles (F): \\Allfiles\\Labs\\03\\Starter\\Images**.
