...
1.  **API Management 서비스** 블레이드에서 다음 작업을 수행합니다.
    ...
    1.  **가격 계층** 목록에서 *소비(99.9 SLA, %)** 을 선택하고 **만들기**을 선택합니다.
...
1.  **새로 만들기** 블레이드에서 **마켓플레이스 검색** 필드를 찾습니다.

1.  검색 필드에서 **논리 앱**를 입력하고 엔터를 선택합니다.
...
1.  **파일 공유** 팝업 대화 상자에서 다음 작업을 수행합니다.
    ...
    1.  **이름** 텍스트 상자에 **metadata**를 입력합니다.
    
    1.  **할당량** 텍스트 상자에 **1**(GiB)을 입력합니다.
    
    1.  **만들기**를 선택합니다.

1.  **파일 공유** 섹션으로 돌아가서 새로 만든 **metadata** 공유를 선택합니다.
...
1.  **논리 앱 디자이너** 블레이드에서 **비어 있는 논리 앱** 템플릿을 선택합니다.

1.  **디자이너** 영역에서 다음 작업을 수행하여 **HTTP 요청을 받았을 때(요청)** 트리거를 추가합니다.
    
    1.  **커넥터 및 트리거 검색** 필드에서 **HTTP**를 입력합니다.(**모두**탭에서 확인합니다.)
    
    1.  변환 목록에서 **요청**을 선택합니다.
    
    1.  **트리거** 결과 목록에서 **HTTP 요청을 수신하는 경우**를 선택합니다.

1.  **HTTP 요청을 수신하는 경우** 영역에서 다음 작업을 수행하여 **HTTP 요청이 수신된 경우(요청)** 트리거를 구성합니다.
    
    1.  **Add new parameter** 목록에서 **메서드**를 선택합니다.
...
1.  **디자이너** 영역에서 **+ 새 단계**를 선택한 다음, 다음 작업을 수행하여 **파일 목록(Azure File Storage)** 작업을 추가합니다.
    ...
    1.  **검색 커넥터 및 트리거** 필드에서 **Azure File Storage**를 입력합니다.
    
    1.  범주 목록에서 **Azure File Storage**를 선택합니다.
    
    1.  **작업** 결과 목록에서 **파일 나열**을 선택합니다.
...
1.  **파일 나열** 영역의 **폴더** 텍스트 상자에 **/metadata**를 입력합니다.
...
1.  **선택** 영역에서 다음 작업을 수행하여 **선택(데이터 작업)** 작업을 구성합니다.
    ...
    1.  **시작** 필드에 있는 **동적 콘텐츠** 목록의 **파일 나열** 범주 내에서 **value**을 선택합니다. 
    
    1.  **지도** 필드에서 오른쪽 끝에 작은 아이콘인 **텍스트 모드로 전환**을 선택합니다.

    1.  **파일 나열** 범주의 **동적 콘텐츠** 목록에 있는 **Name**을 선택합니다.
...
1.  **디자이너** 영역에서 **+ 새 단계**를 선택하고 다음 작업을 수행하여 **응답(요청)** 작업을 추가합니다.
    ...
    1.  **작업** 결과 목록에서 **응답/요청**을 선택합니다.

1.  **응답** 영역에서 다음 작업을 수행하여 **응답(요청)** 작업을 구성합니다.

    1.  **상태 코드** 텍스트 상자에 **200**을 입력합니다.

    1.  **본문**에 있는 **동적 콘텐츠** 필드에서 **선택**목록에서 **출력**을 선택합니다.
...
1.  **API Management** 섹션의 **APIs** 블레이드에서 **API**를 선택합니다.

1.  **Add a new API** 섹션에서 **Logic App**을 선택합니다.

1.  **Create from Logic App** 대화 상자에서 다음 작업을 수행합니다.

    1.  **Full**를 선택합니다.

    1.  **Logic App** 섹션에서 **Browse**을 선택합니다. 
    
    1.  **가져올 논리 앱 선택** 대화 상자에서 이전에 랩에서 만든 **prodflow*[yourname]*** 논리 앱을 선택한 다음 **선택**을 선택합니다.
    
    1.  **Display name** 텍스트 상자에 **Metadata Lookup**를 입력합니다.
    
    1.  **Name** 텍스트 박스에 **metadata-lookup**를 입력합니다.

    1.  **Description** 텍스트 상자에 **Lookup metadata JSON files**를 입력합니다.

    1.  **API URL suffix** 텍스트 상자를 비워 둡니다.

    1.  **Products** 목록에서 **Free**를 선택합니다.
...
1.  **Design** 탭에서 **Test** 탭을 선택합니다.

1.  **Test** 탭에서 다음 작업을 수행합니다.

    1.  단일 **GET** 작업을 선택합니다.

    1.  **Request URL** 필드의 값을 복사합니다. (이 값은 랩에서 나중에 사용합니다.)

    1.  **Send**를 선택합니다.

    1.  **HTTP Response** 섹션에서 테스트 요청의 JSON 결과를 관찰합니다.
    [ ["item_00.json", "item_01.json", "item_02.json", "item_03.json", "item_04.json"] ]
