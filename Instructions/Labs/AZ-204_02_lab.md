...
1.  함수 편집기의 **run.csx** 함수 스크립트에서 예제 코드를 삭제합니다.
...
1.  **테스트/실행**을 선택하여 구성할 수 있는 팝업 대화 상자를 열고 함수 테스트를 수행합니다.

1.  **입력** 탭 내에서 다음 JSON 요청 본문을 입력합니다.
...
1.  \-\-content 옵션을 사용하여 숫자 값 **3**으로 설정된 HTTP 요청 본문에서 보내는 **post** 명령을 실행합니다.

    ```
    post --content 3
    ```

1.  \-\-content 옵션을 사용하여 숫자 값 **5**로 설정된 HTTP 요청 본문에서 보내는 **post** 명령을 실행합니다.

    ```
    post --content 5
    ```

1.  \-\-content 옵션을 사용하여 문자열 값이 **Hello**로 설정된 HTTP 요청 본문에서 전송하는 **post** 명령을 실행합니다.

    ```
    post --content "Hello"
    ```

1.  {"msg":의 JSON 값이 설정된 HTTP 요청 본문에서 전송하는 **post** 명령을 실행합니다. **\-\-content 옵션을 사용하여 "Successful"}**.
...
1.  함수 편집기로 이동하여 **로그**를 선택한 다음, 약 2분마다 발생하는 함수 실행을 관찰합니다. 각 함수 실행은 로그에 간단한 메시지를 렌더링해야 합니다.
...
1.  **트리거 편집** 팝업에서 **일정** 텍스트 상자의 값을 **\*/30 \* \* \* \* \*** 으로 변경합니다.

1.  트리거에 대한 변경 사항을 저장합니다.
...
1.  이 랩의 앞 부분에서 만든 **funcstor*[yourname]*** 스토리지 계정에 액세스합니다.
...
1.  **콘텐츠** 컨테이너에서 **업로드**를 선택하여 **Allfiles (F):** 의 **settings.json** 파일을 업로드합니다.랩 VM의 **\\Allfiles\\Labs\\02\\Starter** 폴더.
...
1.  **App Service** 블레이드에서 **GetSettingInfo** 함수에 대한 **통합** 창을 찾아 엽니다.

1.  **통합** 창에서 다음 구성으로 **Azure Blob Storage** 형식의 새 **입력**을 만듭니다.

    -   Blob 매개 변수 이름: **json**

    -   경로: **content/settings.json**

    -   스토리지 계정 연결: **AzureWebJobsStorage**

1.  **통합** 섹션으로 돌아가서 기존 **HTTP 트리거**를 선택합니다.

1.  **HTTP** 트리거를 업데이트한 다음, 다음 구성으로 업데이트합니다.
...
1.  **IActionResult** 형식의 변수를 반환하고 **HttpRequest** 및 **string** 형식의 변수를 *request* 및 *json*이라는 매개 변수로 사용하는 **Run**이라는 새로운 **public static** 메서드를 만듭니다.
