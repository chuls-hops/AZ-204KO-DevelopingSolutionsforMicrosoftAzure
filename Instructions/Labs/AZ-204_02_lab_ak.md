...
1.  Azure Portal'의 탐색 창에서 **리소스 그룹** 링크를 선택합니다.
...
1. **Functions** 창에서 **추가** 단추를 선택합니다.

1.  **새 함수** 팝업 대화 상자에서 다음 작업을 수행합니다.
    
    1.  **템플릿** 탭 내에서 **HTTP 트리거**를 선택합니다.

    1.  **세부 정보** 탭 내에서 **새 함수** 텍스트 상자를 찾아 **Echo**를 입력합니다.
...
1.  함수 편집기에서 예제 **run.csx** 함수 스크립트를 찾습니다.
...
1.  다음 코드 블록을 추가하여 **IActionResult** 형식의 변수를 반환하고 **HttpRequest** 및 **ILogger** 형식의 변수를 *req* 및 *log*라는 매개 변수로 사용하는 **Run**이라는 새 **공용 정적** 메서드를 만듭니다.
...
1.  **실행** 메서드에서 다음 코드 줄을 입력하여 고정 메시지를 렌더링합니다.
...
1.  함수 코드에 대한 변경 내용을 유지하려면 **저장**을 선택합니다.
...
1.  표시되는 팝업 대화 상자에서 다음 작업을 수행합니다.
    ...
    1.  **입력** 탭의 **본문** 텍스트 상자에 다음의 JSON 요청 본문을 입력합니다.
    ...
    1.  **출력** 탭에서 테스트 실행의 결과를 관찰합니다. 결과는 원래 요청 본문을 정확하게 반복해야 합니다.
...
1.  열린 명령 프롬프트에서 다음 명령을 입력한 다음 Enter 키를 선택하여 기본 URI(Uniform Resource Identifier)를 이 랩의 앞에서 복사한 URL 값으로 설정하는 **httprepl** 도구를 시작합니다.
...
1.  **함수** 블레이드의 **개발자** 섹션에서 **통합** 옵션을 선택합니다.

1.  **통합** 창에서 **타이머 트리거**를 선택합니다.

1.  **트리거 편집** 팝업에서 다음 작업을 수행합니다.

    1.  **예약** 텍스트 상자에서 값을 30으로 변경합니다.
...
1. **함수** 창에서 **+ 추가** 단추를 선택합니다.

1.  **새 함수** 팝업 대화 상자에서 다음 작업을 수행합니다.
    
    1.  **템플릿** 탭 내에서 **HTTP 트리거**를 선택합니다.

    1.  **세부 정보** 탭 내에서 **새 함수** 텍스트 상자를 찾아 **GetSettingInfo**를 입력합니다.
...
1.  **컨테이너** 섹션으로 돌아가서 새로 만든 **콘텐츠** 컨테이너를 선택합니다.
...
1.  **Functions** 창에서 기존의 **GetSettingInfo** 함수를 선택하여 함수에 대한 편집기를 엽니다.

1.  **함수** 블레이드의 **개발자** 섹션에서 **통합** 옵션을 선택합니다.

1.  **통합** 창에서 **+ 입력 추가**를 선택합니다.

1.  **입력 만들기** 팝업 대화 상자에서 다음 작업을 수행합니다.

    1.  **바인딩 유형** 목록에서 **Azure Blob Storage**를 선택합니다.

    1.  **Blob 매개 변수 이름** 텍스트 상자에 **json** 값을 입력합니다.

    1.  **경로** 텍스트 상자에 **content/settings.json** 값을 입력합니다.

    1.  **Storage 계정 연결** 목록에서 **AzureWebJobsStorage**를 선택합니다.

    1.  **확인**을 선택합니다.

1.  **통합 창**으로 돌아가서 **HTTP** 트리거를 선택합니다.

1.  **HTTP 트리거** 팝업 대화 상자에서 다음 작업을 수행합니다.

    1.  **매개 변수 이름 요청** 텍스트 상자에 **요청** 값을 입력합니다.

    1.  **선택한 HTTP 메서드** 확인란 그룹에서 **GET ** 옵션만 선택되어 있는지 확인합니다.
...
1.  다음 코드 블록을 추가하여 **Run**이라는 새 **공용 정적** 메서드를 만드는데, 이 메서드는**IActionResult** 형식의 변수를 반환하고 **httpRequest** 및 **스트링**의 변수를 *요청* 및 *json이라는* 매개 변수로 사용합니다.
