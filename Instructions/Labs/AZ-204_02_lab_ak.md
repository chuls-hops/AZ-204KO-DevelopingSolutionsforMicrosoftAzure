...
1.  **호스팅** 탭에서 다음 작업을 수행합니다.
    ...
    1.  **계획 유형** 드롭다운 목록에서 **소비** 옵션을 선택합니다.
...
1.  **새 Azure 함수** 빠른 시작에서 다음 작업을 수행합니다.
    ...
    1.  **새 함수** 팝업 창에서 **이 름** 텍스트 상자를 찾아 **Echo** 를 입력합니다.
...
1.  다음 코드 블록을 추가하여 **IActionResult** 형식의 변수를 반환하고 **HttpRequest** 및 **ILogger** 형식의 변수를 *req* 및 *log* 라는 매개 변수로 사용하는 **Run** 이 라는 새 **공용 정적** 메서드를 만듭니다.
...
1.  컴파일 결과를 관찰합니다. 결과에는 "컴파일 성공" 메시지가 포함되어야 합니다.
...
1.  **함수 앱** 블레이드에서 **URL** 텍스트 상자의 값을 복사합니다. 이 값은 랩에서 나중에 사용합니다.
...
1.  **새 Azure 함수** 빠른 시작에서 다음 작업을 수행합니다.
    ...
    1.  **새 함수** 팝업 창에서 ** 이 름** 텍스트 상자를 찾아 **Recurring** 을 입력합니다.
    
    1.  **새 함수** 팝업 창에서 **일정** 텍스트 상자를 찾아 **0 \* \* \* \* \*** 를 입력합니다.
    
    1.  **새 함수** 팝업 창에서 **만들기** 를 선택합니다.

#### 작업 2: 함수 실행 관찰

1.  함수 편집기에서 기본 함수 구현을 유지하려면 **저장** 을 선택합니다.
...
1.  **함수 앱** 블레이드에 돌아와서 다음 작업을 수행합니다.
    ...
    1.  이 랩의 앞에서 만든 **funclogic*[사용자 이름]*** 함수 앱에 대한 노드를 펼칩니다.

    1.  **함수** 노드를 펼칩니다.

    1.  특정 함수에 대한 **되풀이** 노드를 확장합니다.

    1.  **통합** 노드를 선택합니다.

1.  **통합** 섹션에서 다음 작업을 수행합니다.

    1.  **트리거** 섹션에서 **타이머(myTimer)** 옵션을 선택합니다.

    1.  **일정** 텍스트 상자에 **\*/30 \* \* \* \* \*** 값을 입력합니다.
...
1.  **새 Azure 함수** 빠른 시작에서 다음 작업을 수행합니다.
    ...
    1.  **새 함수** 팝업 창에서 **이 름** 텍스트 상자를 찾아 **GetSettingInfo** 를 입력합니다.
...
1.  **서버리스** 블레이드에서 이 랩의 앞에서 만든 **funcstor*[사용자 이름]*** 스토리지 계정을 선택합니다.
...
1.  **Function Apps** 블레이드에서 다음 작업을 수행합니다.
    ...
    1.  특정 함수에 대해 **GetSettingInfo** 노드를 펼칩니다.
...
1.  **Azure Blob Storage 입력** 섹션에서 다음 작업을 수행합니다.
...
1.  **HTTP 트리거** 섹션에서 다음 작업을 수행합니다.
    ...
    1.  **허용된 HTTP 메서드** 목록에서 **선택된 메서드** 를 선택합니다.
...
1.  **Function Apps** 블레이드에서 다음 작업을 수행합니다.
    ...
    1.  특정 함수에 대해 **GetSettingInfo** 노드를 선택합니다.
