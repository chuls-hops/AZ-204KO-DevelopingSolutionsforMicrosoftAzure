...
1.  Azure Portal'의 탐색창에서 **리소스 만들기** 링크를 선택합니다.
...
1.  **새 함수** 팝업 대화 상자에서 다음 작업을 수행합니다.
    ...
    1.  **세부 정보** 탭 내에서 **새 함수** 텍스트 상자를 찾아 **FileParser**를 입력합니다.
...
1.  표시되는 팝업 대화 상자에서 다음 작업을 수행합니다.
    ...
    1.  **출력** 탭에서 "테스트 성공"의 출력 메시지를 관찰합니다.

#### 작업 3: Key Vault에서 파생된 애플리케이션 설정 테스트
...
1.  다음 코드 줄을 추가하여 **Environment.GetEnvironmentVariable** 메서드를 통해 **StorageConnectionString** 애플리케이션 설정의 값을 얻습니다:

    ```
    string connectionString = Environment.GetEnvironmentVariable("StorageConnectionString");
    ```

1.  다음 코드 줄을 추가하여 **OkObjectResult** 클래스 생성자를 사용하여 *connectionString* 변수의 값을 반환합니다.
...
1.  표시되는 팝업 대화 상자에서 다음 작업을 수행합니다.
    ...
    1.  **출력** 탭에서 함수에서 반환된 출력 연결 문자열을 관찰합니다.
...
1.  대화 상자에 **function.proj**를 입력한 다음 Enter를 선택하면 빈 코드 편집기가 표시됩니다.
...
1.  편집기에서 이 구성 내용을 삽입합니다.
    ...
    > **참고**: App Service 편집기에서 파일에 대한 변경 사항을 자동으로 저장합니다. 이 .proj 파일에는 [Azure.Storage.Blobs](https://www.nuget.org/packages/Azure.Storage.Blobs/12.4.0) 패키지를 가져오는 데 필요한 NuGet 패키지 참조가 포함되어 있습니다.

1.  **App Service 편집기**로 브라우저 창을 닫습니다.
...
1.  **Functions** 창에서 기존의 **FileParser** 함수를 선택하여 함수에 대한 편집기를 엽니다.
...
1.  다음 코드 줄을 추가하면 **FileStreamResult** 클래스 생성자를 사용하여 *콘텐츠* 변수에 저장된 다양한 콘텐츠를 반환합니다.
...
1.  표시되는 팝업 대화 상자에서 다음 작업을 수행합니다.
    ...
    1.  **출력** 탭에서 스토리지 계정에 저장된 **$/drop/records.json** Blob의 출력 콘텐츠를 관찰합니다.
