...
# 랩: 서비스 전반에서 리소스 비밀에 보다 안전하게 액세스
# 학생 랩 답변 키
...
1.  **마켓플레이스** 검색 결과 블레이드에서 **함수 앱** 결과를 선택합니다.

1.  **함수 앱** 블레이드에서 **만들기**를 선택합니다.

1.  **기본**과 같은 **함수 앱** 블레이드에서 탭을 찾습니다.
...
1.  **설정** 블레이드에서 **ID** 탭을 선택합니다.

1.  **ID** 상단메뉴에서 **시스템 할당 항목** 탭을 찾은 후 다음 작업을 수행합니다.
...
1.  **비밀 만들기** 블레이드에서 다음 작업을 수행합니다.
    ...
    1.  **콘텐츠 형식(선택 사항)** 텍스트 상자를 기본값으로 둡니다.
    
    1.  **활성화 날짜를 설정하시겠습니까?** 체크 박스를 기본값으로 설정한 상태로 둡니다.
    
    1.  **만료 날짜를 설정하시겠습니까?** 체크 박스를 기본값으로 설정된 상태로 둡니다.
    
    1.  **사용하도록 설정되어 있습니까?** 섹션에서 **예**를 선택한 다음 **만들기**를 선택합니다. 
...
1.  비밀 버전 창에서 다음 작업을 수행합니다.
    ...
    > **참고**: **비밀 값** 텍스트 상자가 아닌 **비밀 식별자** 텍스트 상자의 값을 기록해 둡니다.
...
1.  **액세스 정책 추가** 블레이드에서 다음 작업을 수행합니다.
    ...
    1.  **권한 있는 애플리케이션** 선택 상자를 기본값으로 설정합니다.
...
1.  **구성** 섹션에서 다음 작업을 수행합니다.
    ...
    1.  **배포 슬롯 설정** 텍스트 박스를 기본값으로 설정한 상태로 둡니다.
...
1.  **함수** 블레이드에서 **함수** 선택, 상단메뉴에서 **+추가**를 선택합니다.

1.  **새 함수** 빠른 시작에서 다음 작업을 수행합니다.
    
    1.  템플릿 목록에서 **HTTP trigger**를 선택합니다.
    
    1.  **새 함수** 팝업 창에서 **새 함수** 텍스트 상자를 찾아 **FileParser**를 입력합니다.
    
    1.  **새 함수** 팝업 창에서 **Authorization level** 목록을 찾아 **Anonymous**을 선택합니다.
    
    1.  **새 함수** 팝업 창에서 **함수 만들기**를 선택합니다.

1. ***코드+테[스트*에서 함수 예제 스크립트를 확인합니다.
...
1.  예제 코드를 모두 **삭제**한 다음 함수 편집기에서 다음 내용의 함수를 복사하여 붙여 넣습니다.
...
1.  **저장**을 선택하여 스크립트를 저장하고 상단 메뉴에서 **Test/Run**을 선택하고 **실행** 합니다.

1.  스크립트가 처음으로 실행될 때 테스트 및 로그 창이 자동으로 나타납니다.

    > **참고**: 로그를 컴파일하는 동안 경고 내용이 나타날 수 있습니다. 무시해도 됩니다.

1.  테스트 창에서 **출력** 텍스트 상자를 찾습니다. 이제 함수에서 반환된 **Test Successful** 값이 표시됩니다.
...
1.  **새 컨테이너** 팝업 창에서 다음 작업을 수행합니다.
    ...
    1.  **이름** 텍스트 상자에 **drop**를 입력합니다.
    
    1.  **공용 액세스 수준** 드롭다운 목록에서 **Blob(Blob에 대한 익명 읽기 전용 액세스)** 을 선택한 후 **확인**을 선택합니다.
...
1.  이제 Blob의 JSON(JavaScript Object Notation) 내용이 표시됩니다. JSON 내용을 확인하고 브라우저 창을 닫습니다.
...
1. **설정** 섹션에서 **구성** 옵션을 선택합니다.

1. **구성** 창에서 다음 작업을 수행하십시오.
    
    1. **애플리케이션 설정** 탭을 선택한 다음 **새 애플리케이션 설정**을 선택합니다.
    
    1. **애플리케이션 설정 추가/편집** 팝업 창의 **이름** 텍스트 상자에 **DOTNET_ADD_GLOBAL_TOOLS_TO_PATH**를 입력합니다.
    
    1. **값** 텍스트 상자에 **false**를 입력하십시오.
    
    1. **배포 슬롯 설정** 텍스트 상자를 기본값으로 설정하십시오.

    1. **확인**을 선택하여 팝업 창을 닫고 **구성** 섹션으로 돌아갑니다.
    
    1. 블레이드에서 **저장**을 선택하여 설정을 유지합니다.

    1. **변경 사항 저장** 확인 팝업 대화 상자에서 **계속**을 선택하십시오.

    > **참고** : 실습을 진행하기 전에 응용 프로그램 설정이 유지 될 때까지 기다리십시오.
    
#### 작업 3 : NuGet에서 저장소 계정 SDK 가져 오기

1. Azure Portal의 탐색 창에서 **리소스 그룹** 링크를 선택합니다.

1. **리소스 그룹** 블레이드에서 앞서 생성 한 **SecureFunction** 리소스 그룹을 찾아 선택합니다.

1. **SecureFunction** 블레이드에서 앞서 생성 한 **securefunc*[yourname]*** 기능 앱을 선택합니다.

1. **개발 도구** 섹션에서 **App Service 편집기(미리보기)** 옵션을 선택합니다.

1. **App Service 편집기(미리보기)** 창에서 **이동->**을 선택합니다.

1. **App Service Editor** 브라우저 창에서 **FileParser** 폴더를 확장하고 마우스 오른쪽으로 메뉴를 연 다음 **New file**을 선택합니다.

1. 대화 상자에서 **function.proj**를 입력 한 다음 Enter를 선택하면 빈 코드 편집기가 표시됩니다.

1. 편집기에서이 구성 컨텐츠를 삽입합니다.
...
1.  편집기창 오른쪽 위쪽에 **SAVED**를 확인한다.(자동 저장이 된다.)

1. **App Service Editor**로 브라우저 창을 닫습니다.


#### 작업 4 : 네임 스페이스 참조 업데이트
...
1. **함수** 섹션에서 **함수** 옵션을 선택합니다.

1. **함수** 창에서 기존 **FileParser** 기능을 선택하여 해당 기능의 편집기를 엽니다.

1. **개발자** 섹션에서 **코드 + 테스트** 옵션을 선택합니다.

1. 편집기 내에서 스크립트의 ** Run ** 메소드에서 기존 코드를 삭제하십시오.

1. 코드 파일에서 다음 코드 줄을 추가하여 ** Azure.Storage ** 네임 스페이스에 대한 ** using ** 지시문을 만듭니다.
...
1.  이제 다음을 포함해야 하는 **Run** 메서드를 관찰합니다.
...
1.  **Run** 메서드에서 다음 코드 줄을 추가하고 **Environment.GetEnvironmentVariable** 메서드를 사용하여 **StorageConnectionString** 애플리케이션 설정의 값을 얻습니다.
...
1. 함수 코드에 대한 변경 사항을 유지하려면 **저장**을 선택합니다.

1. **Test/Run**을 선택합니다.

1. 팝업 대화 상자가 나타나면 다음 작업을 수행하십시오.

     1. **입력** 탭에서 **실행**을 선택합니다.

     1. **출력** 탭에서 스토리지 계정에 저장된 **$/drop/records.json** Blob의 출력 내용을 확인하십시오.
